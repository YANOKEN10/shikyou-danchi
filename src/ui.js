...
export class UI {
  ...
  async showEnding(ending, stats, onAgain) {
    // 脱出後は生成済みの外景を最後まで固定し、扉や歩行の別アニメーションへ切り替えない。
    this.el.ending.classList.toggle("exterior", !ending.bad);
    if (!ending.bad) this.snd.endingDawn(Boolean(ending.best));
  ...
  closeEnding() {
    this.el.ending.classList.remove("show", "exterior");
    this.open = null;
  }
