...
## 2026-09-15 利用状況の計測
管理ページ用の usage-summary API と usage-tracker を追加。既存セーブ・認証データは変更しない。STATS_READ_KEYで集計APIを保護。プレイ時間は表示中かつ操作後60秒以内、30秒送信。現在人数は直近90秒の推定。登録数はゲストを含むアカウント数。

## 2026-09-20 Codex: 計測除外
利用状況管理ページの本人依頼で /usage-settings.html を追加。yg-usage-excluded=1 のブラウザは利用人数・ログイン人数・プレイ時間を今後送信しない。過去記録・登録数は保持。各端末/ブラウザ/ゲームごとに設定。現在の本番版へ計測4ファイルだけ反映（公開方式に応じてtrackerタグも維持）。公開配信一致確認。計測ファイルを次回デプロイにも含めること。

## 2026-09-20 Codex: 集計APIの復旧
本番で404となっていた /api/usage-summary を復旧。計測スクリプト・読込タグ・端末別除外画面もGit管理に含め、次回のGit連携デプロイで欠落しないようにする。登録数のみを返す認証付き読み取りAPI。認証・複数ページ・取得失敗時のテスト合格。ゲーム内容・既存データは変更しない。
